...
| `urn` | String | generated, value and format depend on the persistence layer implementation | no default | will be generated if not provided
...
| `sortBy` | query | String | chronological order | paged responses sorted by this JSON field name
|====================
...
Create a new Thing belonging to the Tenant of the authenticated User, and its associated
Metadata. In the first example, the fields `name`, `description` and `large` will be
...
is only useful for turning the _active_ flag off and on for the Thing itself, and
for updating the Thing's Metadata.
...
Get all Things of the specified type belonging to the Tenant of the authenticated
User, and their associated Metadata. If the `fields` query parameter is set,
the result will include only the Metadata fields specified. If the `fields`
query parameter is not set, all Metadata for the returned Things will be included
in the result. Results are paged.
